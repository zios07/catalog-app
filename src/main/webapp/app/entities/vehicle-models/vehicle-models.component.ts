...
export class VehicleModelsComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.vehicleModelsService
      ...
      .subscribe(
        (res: HttpResponse<IVehicleModels[]>) => this.paginateVehicleModels(res.body, res.headers),
  ...
  trackId(index: number, item: IVehicleModels) {
    return item.id;
  }

  registerChangeInVehicleModels() {
    this.eventSubscriber = this.eventManager.subscribe('vehicleModelsListModification', response => this.loadAll());
  }
  ...
  protected paginateVehicleModels(data: IVehicleModels[], headers: HttpHeaders) {
