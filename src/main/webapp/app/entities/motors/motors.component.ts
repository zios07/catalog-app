...
export class MotorsComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.motorsService
      ...
      .subscribe(
        (res: HttpResponse<IMotors[]>) => this.paginateMotors(res.body, res.headers),
  ...
  trackId(index: number, item: IMotors) {
    return item.id;
  }

  registerChangeInMotors() {
    this.eventSubscriber = this.eventManager.subscribe('motorsListModification', response => this.loadAll());
  }
  ...
  protected paginateMotors(data: IMotors[], headers: HttpHeaders) {
